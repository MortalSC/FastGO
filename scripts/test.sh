...
# 随机生成用户名
fg::test::username() {
    echo "fastgo$(date +%s%N | sha256sum | base64 | head -c 8)"
}
...
fg::test::user()
{
  ...
  # 1. 创建 fastgo 用户
  ${CCURL} "${Header}" http://${INSECURE_SERVER}/api/v1/user \
  ...
  # 2. 列出所有用户
  ${RCURL} "${token}" "http://${INSECURE_SERVER}/api/v1/user?offset=0&limit=10"; echo
  echo -e "\033[32m2. 成功列出所有用户\033[0m"

  # 3. 获取 fastgo 用户的详细信息
  ${RCURL} "${token}" http://${INSECURE_SERVER}/api/v1/user/${username}; echo
  echo -e "\033[32m3. 成功获取 ${username} 用户详细信息\033[0m"

  # 4. 修改 fastgo 用户
  ${UCURL} "${Header}" "${token}" http://${INSECURE_SERVER}/api/v1/user/${username} \
    -d'{"nickname":"fastgo(modified)"}'; echo
  echo -e "\033[32m4. 成功修改 ${username} 用户信息\033[0m"

  # 5. 删除 fastgo 用户
  ${DCURL} "${token}" http://${INSECURE_SERVER}/api/v1/user/${username}; echo
...
fg::test::post()
{
  ...
  username=$(fg::test::username)
  # 1. 创建测试用户
  ${CCURL} "${Header}" "${token}" http://${INSECURE_SERVER}/api/v1/user \
    -d'{"username":"'${username}'","password":"fastgo1234","nickname":"fastgo","email":"colin404@foxmail.com","phone":"'$(date +%s)'"}'; echo
  ...
  # 2. 创建一条博客
  postID=`${CCURL} "${Header}" "${token}" http://${INSECURE_SERVER}/api/v1/post -d'{"title":"installation","content":"installation."}' | grep -Po 'post-[a-z0-9]+'`
  echo -e "\033[32m2. 成功创建博客: ${postID}\033[0m"

  # 3. 列出所有博客
  ${RCURL} "${token}" http://${INSECURE_SERVER}/api/v1/post; echo
  echo -e "\033[32m3. 成功列出所有博客\033[0m"

  # 4. 获取所创建博客的信息
  ${RCURL} "${token}" http://${INSECURE_SERVER}/api/v1/post/${postID}; echo
  echo -e "\033[32m4. 成功获取博客 ${postID} 详细信息\033[0m"

  # 6. 修改所创建博客的信息
  ${UCURL} "${Header}" "${token}" http://${INSECURE_SERVER}/api/v1/post/${postID} -d'{"title":"modified"}'; echo
  echo -e "\033[32m5. 成功更新博客 ${postID} 信息\033[0m"

  # 7. 删除所创建的博客
  ${DCURL} "${token}" http://${INSECURE_SERVER}/api/v1/post -d'{"postIDs":["'${postID}'"]}'; echo
